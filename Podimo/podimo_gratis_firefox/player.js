...
function injectStyles() {
    ...
    if (!document.getElementById(styleId)) {
        ...
        link.id = styleId;
        link.rel = 'stylesheet';
        link.href = (typeof browser !== 'undefined' ? browser.runtime.getURL('player.css') : './player.css');
...
async function loadShaka() {
    if (!window.shaka) {
        throw new Error('Shaka Player ikke indlæst');
    }
    
    if (!shaka.Player.isBrowserSupported()) {
        throw new Error('Shaka Player understøttes ikke i denne browser');
    }
    ...
    try {
        ...
        if (navigator.userAgent.includes('Firefox')) {
            try {
                if (shaka.polyfill.MediaSourceWorkarounds) {
                    shaka.polyfill.MediaSourceWorkarounds(window);
                }
            } catch (polyfillErr) {
                console.warn('Firefox polyfills failed:', polyfillErr);
            }
        }
        
        return shaka.Player;
    } catch (err) {
        console.error('Shaka polyfill error:', err);
        // Don't throw - return Player anyway for basic functionality
        return shaka.Player;
    }
}
...
async function setMediaSource(mediaElement, url, isVideo) {
    if (!isVideo) {
        ...
        mediaElement.load(); // Add load() for consistency
        return;
    }
    ...
    try {
        ...
        if (player && player.destroy) {
            try {
                ...
                console.warn('Player cleanup failed:', destroyErr);
            }
        }
        
        mediaElement.src = '';
        mediaElement.load();
        throw new Error('HLS kræver Shaka Player i Firefox - genindlæs siden');
    }
}
